> **AI작업 배치(260718)**: 작업 기록은 **그 서버 홈 바로 아래 `/home/k/AI작업/` 한 곳**에 둔다(작업일지·현재작업.d·locks.d·해야하는작업.md). 서비스/repo별 폴더로 흩지 말 것 — 서버당 한 곳이 원칙. 유일한 예외는 repo별 `version.md`(코드 변경 이력). 전 서버 현황은 **k1이 단일 인덱스로 집계**한다 — 각 서버는 로컬에 쓰기만 하고, 여러 서버를 가로질러 보는 읽기는 k1 한곳에서 한다. 상세 표준은 `/home/k/AI작업/협업규칙.md`.
>
> **새 서버·새 서비스 시작 시(부트스트랩)**: ① 그 서버에 `/home/k/AI작업/{작업일지,현재작업.d,locks.d}`가 없으면 만든다. ② 새 서버면 k1 fleet 목록 `boss/전서버제어/초기설정/ai-cli-fleet/hosts.tsv`에 등록해 k1 감사·인덱스 대상에 포함시킨다. ③ 이 블록이 그 서버 홈의 `CLAUDE.md`·`AGENTS.md`에 있는지 확인하고, 없으면 k1의 전파 스크립트를 돌린다.

> **행동수칙(260716)**: ① 요청이 여러 해석이 가능하면 말없이 하나 고르지 말 것 — 간단하면 물어보고, 자율 진행 중이면 택한 해석과 이유를 보고에 명시한다. ② 요청받은 방식보다 확실히 단순한 방법이 있으면 구현 전에 말한다. 근거가 있으면 반대 의견도 낸다.
