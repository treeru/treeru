...
### [**⬇ TreeRU 다운로드**](https://github.com/treeru/treeru/releases) &nbsp;·&nbsp; [![latest release](https://img.shields.io/github/v/release/treeru/treeru?label=latest&color=2ea44f)](https://github.com/treeru/treeru/releases)
...
- **zellij 스타일 탭바** — 화살표 리본 + 반응형 폭(공간이 남으면 풀네임, 모자랄 때만 축소). 파워라인 글리프 없는 폰트는 `~/.treeru_config.json`에 `"tabStyle": "chip"`
...
2. TreeRU가 현재 폴더에 `screenshot_....png`로 자동 저장하고 **경로를 클립보드에 복사**
3. Claude Code로 전환 → `Ctrl+V` → 이미지를 읽고 분석합니다

(자동 복사가 꺼져 있다면 기본값은 켜짐 — `~/.treeru_config.json`에서 `"screenshotCopyPath": true`. 아무 파일에서 `Alt+Shift+C`로 직접 경로 복사도 언제든 가능합니다.)
...
IDE 없이, 마우스 씨름 없이. 경로 복사해서 붙여넣기만 하면 됩니다.
...
| `PrtSc` / `Win+Shift+S` | 스크린샷 촬영 → 현재 폴더에 자동 저장 + 경로 클립보드 복사 |
| `Esc` | 선택 해제 |
...
1단 `ProxyJump`를 지원하므로 bastion 뒤의 내부 서버도 OpenSSH와 같이 접속할 수 있습니다. 다중 또는 중첩 점프 호스트는 지원하지 않습니다.
...
## 다른 터미널 앱과 나란히 쓰기 (zellij / tmux / vim)

TreeRU는 마우스 추적을 켜는데, 터미널은 이 마우스 신호를 포커스된 앱으로 이스케이프 시퀀스로 전달합니다. 그래서 같은 터미널 창 안에서 다른 풀스크린 앱(예: `ssh` + `zellij`)과 TreeRU를 동시에 띄우면, 그 마우스 신호가 다른 앱으로 새어 들어가 이상한 문자로 보일 수 있습니다. 두 가지 회피법:

- **TreeRU를 별도 터미널 창에서 실행** (다른 앱의 분할 창/탭이 아니라 아예 다른 창) — 완전히 분리됩니다.
- **TreeRU 마우스 끄기**: `~/.treeru_config.json`에 `"mouse": false`. 그러면 TreeRU가 키보드 전용으로 동작하고 마우스 시퀀스를 전혀 안 내보내서, 어떤 터미널 앱과도 충돌 없이 공존합니다.

TreeRU는 종료·크래시 시 터미널을 완전히 복구(마우스 끄기, 커서 복원)하므로, 마우스 모드가 뒤에 남는 일도 없습니다.
